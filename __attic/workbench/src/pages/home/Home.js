...
export const Home = () => {
  ...
  const handleContentTypeChange = async (newContentType) => {
    if (newContentType) {
      ...
      if (newContentType === 'application/did+cbor') {
        const result = await refresh(keys, 'application/did+ld+json');
        setKeys(result.keys);
        setResolutionResponse(borc.encode(result.resolutionResponse));
      } else {
  ...
  const handleGenerate = async (index = tabValue) => {
    ...
    if (contentType === 'application/did+cbor') {
      const result = await refresh(null, 'application/did+ld+json');
      setKeys(result.keys);
      setResolutionResponse(borc.encode(result.resolutionResponse));
    } else {
